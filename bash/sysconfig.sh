...
function error-exit {
error-message >&2; 
#Not using exit 1 here because it will close the shell. return is more suitable for this task.
return 1
}
...
function displayhelp {
  ...
  echo "--ipconfig    displays an ip info"
  echo "--os          displays the operating system info"
  echo "--cpu         displays the cpu info"
  echo "--memory      displays the memory info"
  echo "--disk        displays a disk info"
  echo "--printer     displays the printer info"
}
...
function cleanup {
	rm -fr /tmp/*info*
	echo "SIGINT execution finished."
}
trap cleanup SIGINT
